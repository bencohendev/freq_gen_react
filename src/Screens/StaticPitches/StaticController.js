...
function reducer(nodes, action) {
  ...
  switch (action.type) {
    case 'create':
      action.i ? action.i = action.i : action.i = 0
    ...
    switch (selectedPreset) {
      case '1 - 3 - 5':
        ...
        freqArray.map((freq, i) => {
          return dispatch({type: 'create', freq, i})
  ...
  const actualPlayPause = (node, i) => {
    console.log('actualplaypause')
    console.log(node.playing)
    if(node.playing === 'Play') {
      dispatch({type: 'play', i})
    } else if (node.playing === 'Pause')
